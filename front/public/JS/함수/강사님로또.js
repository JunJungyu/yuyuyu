...
function lotto_number(){
	...
	// 결과 함수 출력 
	lotto_result()
	let count = 0 // 총 동일한 숫자의 개수
}
// 4. 추첨번호 비교 결과 함수 
function lotto_result(){
	for( let i of select_numbers ){
		for(let j of select_number ){
			if(i==j){
				count++	// 1증가
			}
		}
		//for ( 변수명 of 배열명 ) : 배열 내 요소를 하나씩 꺼내온다. 변수에 대입반복
		}	//for e
		if(lotto_numbers.indexOf(i) >= 0){count++;}
		
		//3. fliter() : 새로운 배열 생성[ return이 ture인 경우에만 저장 ] / 배열명.includes 값 : 해당 배열에 값이 존재하면 ture 반환
		let result_nimbers = select_numbers((value)=>lotto_numbers.includes(value))
		count = select_numbers.fliter( (value) => lotto_numbers.includes(value)).lenght	
		alert( count )
		let msg = ''	//출력할 메세지를 저장하는 변수
		if(count == 0 ){msg += 꽝}
		else if(count == 1){msg += '1개 맞았습니다.'}
		else if(count == 1){msg += '2개 맞았습니다. [5등]'}
		else if(count == 1){msg += '3개 맞았습니다. [4등]'}
